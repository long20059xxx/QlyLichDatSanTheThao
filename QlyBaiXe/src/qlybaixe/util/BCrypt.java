...
public class BCrypt {
    ...
    static {
        ...
        for (int i = 0; i < BASE64_CODE.length(); i++) {
            INDEX_64[BASE64_CODE.charAt(i)] = i;
        }
    }
    ...
    private byte[] cryptRaw(byte[] password, byte[] salt, int logRounds, int[] cdata) {
        int rounds = 1 << logRounds;
        byte[] ret;
        int clen = cdata.length;
        int[] cdataCopy = cdata.clone();
        ...
        for (int i = 0; i < 64; i++) {
            for (int j = 0; j < (clen >> 1); j++) {
                encipher(cdataCopy, j << 1);
            }
        }

        ret = new byte[clen * 4];
        for (int i = 0, j = 0; i < clen; i++) {
            ret[j++] = (byte) ((cdataCopy[i] >> 24) & 0xff);
            ret[j++] = (byte) ((cdataCopy[i] >> 16) & 0xff);
            ret[j++] = (byte) ((cdataCopy[i] >> 8) & 0xff);
            ret[j++] = (byte) (cdataCopy[i] & 0xff);
        }
        return ret;
    }
    ...
    public static String hashpw(String password, String salt) {
        ...
        if (salt.charAt(2) == '$') {
            ...
            minor = String.valueOf(salt.charAt(2));
            if ((minor.charAt(0) != 'a' && minor.charAt(0) != 'b' && minor.charAt(0) != 'y') || salt.charAt(3) != '$') {
        ...
        passwordBytes = (password + (minor.equals("a") || minor.equals("b") || minor.equals("y") ? "\000" : "")).getBytes(java.nio.charset.StandardCharsets.UTF_8);
        saltBytes = decodeBase64(realSalt, BCRYPT_SALT_LEN);

        hashed = bCrypt.cryptRaw(passwordBytes, saltBytes, rounds, BF_CRYPT_CIPHERTEXT);
    ...
    public static boolean checkpw(String plaintext, String hashed) {
        return slowEquals(hashed.getBytes(java.nio.charset.StandardCharsets.UTF_8),
                hashpw(plaintext, hashed).getBytes(java.nio.charset.StandardCharsets.UTF_8));
    }
